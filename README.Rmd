...
We have a lot of ASVs assigned to Eukarya, so let's get rid of those first. Also notice that our ASV IDs are sequences, which can be unwieldy to deal with, and can cause issues with memory usage for really large data sets. Finally, there are NA assignments in the Features table. So we're going to run a couple steps to clean things up.
